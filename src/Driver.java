...
public class Driver<T extends Transport & Competing> {
    ...
    public void setCurrentTransport(T transport) {
        if (transport.getClass() == Car.class && Arrays.asList(licenses).contains("B")) currentTransport = transport;
        else if (transport.getClass() == Truck.class && Arrays.asList(licenses).contains("C")) currentTransport = transport;
        else if (transport.getClass() == Bus.class && Arrays.asList(licenses).contains("D")) currentTransport = transport;
        else System.out.println("У водителя "+getFullName()+" нет лицензии на данный транспорт");
    }
